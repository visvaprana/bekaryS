            ...
            @foreach($orders as $order)
                ...
                @php
                    ...
                    $total_amount = $total_amount + $order->qty_total_amount;
                      ...
                      // Calculate received amount with discount
                        $mainOrder = App\Models\Order::find($order->order_id);
                        $totalOrderAmount = App\Models\OrderDetail::where('order_id', $order->order_id)->sum('qty_total_amount');
                        $discount = $mainOrder->discount ?? 0;

                        // Calculate proportional discount
                        $proportionalDiscount = 0;
                        if ($totalOrderAmount > 0) {
                            $proportionalDiscount = ($order->qty_total_amount / $totalOrderAmount) * $discount;
                        }

                        $receivedAmount = $order->qty_total_amount - $proportionalDiscount;
                        $total_received += $receivedAmount; // Add this amount to running total
                ...
                <tr>
                    ...
                    <td>
                        @php
                            ...
                            $receivedAmount = $order->qty_total_amount - $proportionalDiscount;
                        @endphp
                        {{ number_format($receivedAmount, 2) }} BDT
                    </td>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td></td>
                      <td><strong>Total Received: {{$total_received ?? ''}} TK</strong></td>
                    <td></td>
